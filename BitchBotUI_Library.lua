--[[
    BitchBot UI Library - Extracted from original source
    1:1 UI system (Draw, menu.Initialize, mouse, tabs, controls)
    Load with: local menu = loadstring(...)()
    Then call menu.Initialize({...}) after setup.
]]
...
if not BBOT then BBOT = { username = "dev" } end
BBOT_IMAGES = BBOT_IMAGES or {"","","","",""}

-- minimal event system used by original menu
local event = {}
function event.new(name)
    ...
    return {
        ...
        Connect = function(_, fn) table.insert(listeners, fn) return {Disconnect=function() end} end,
        fire = function(_, ...) for _,fn in ipairs(listeners) do pcall(fn, ...) end end,
        Fire = function(_, ...) for _,fn in ipairs(listeners) do pcall(fn, ...) end end,
    }
end
...
local DOUBLE_COLORPICKER = 9 -- alias used in original
local SLIDER = 10
local BUTTON = 11
local LIST = 12
local IMAGE = 13
...
pcall(function()
	if crypt and crypt.base64decode then placeholderImage = crypt.base64decode(placeholderImage)
	elseif base64_decode then placeholderImage = base64_decode(placeholderImage)
	end
end)
		...
		for k, v in pairs(allrender) do
			for k1, v1 in pairs(v) do
				if v1 and type(v1) ~= "number" then
					local exists = true
					pcall(function()
						if v1.__OBJECT_EXISTS == false then exists = false end
					end)
					if exists then
						if not pcall(function() v1:Remove() end) then
							pcall(function() v1:Destroy() end)
						end
					end
				end
			end
...
function menu.Initialize(menutable)
					...
					if content ~= nil then
						for k2, v2 in pairs(content) do
							...
							elseif v2.type == COMBOBOX then
								...
								menu.options[v.name][g_name][v2.name][3] = { v1.x + 7, v1.y + y_pos - 1, v1.width - 16 }
								menu.options[v.name][g_name][v2.name][5] = false
								menu.options[v.name][g_name][v2.name][6] = v2.values -- required for mouse handlers
								y_pos += 40
	...
	function menu:GetVal(tab, groupbox, name, ...)
		...
		if not menu.options or not menu.options[tab] or not menu.options[tab][groupbox] then
			return nil
		end
			...
			for k1, v1 in pairs(v) do
				for k2, v2 in pairs(v1) do
					if v2[2] == DROPBOX and v2[5] then
						if not menu:MouseInMenu(v2[3][1], v2[3][2], v2[3][3], 24 * ((type(v2[6])=="table" and #v2[6] or 0) + 1) + 3) then
							menu:SetDropBox(false, 400, 200, 160, 1, { "HI q", "HI q", "HI q" })
							v2[5] = false
						else
						...
						if pass then
							for k2, v2 in pairs(v1) do
								...
								elseif v2[2] == DROPBOX then
									...
									elseif menu:MouseInMenu(v2[3][1], v2[3][2], v2[3][3], 24 * ((type(v2[6])=="table" and #v2[6] or 0) + 1) + 3) and v2[5]
									then
										for i = 1, (type(v2[6])=="table" and #v2[6] or 0) do
											if menu:MouseInMenu(
													v2[3][1],
													v2[3][2] + 36 + ((i - 1) * 21),
													v2[3][3],
...
-- Export constants on menu for examples
menu.COLOR = COLOR
menu.TOGGLE = TOGGLE
menu.KEYBIND = KEYBIND
menu.DROPBOX = DROPBOX
